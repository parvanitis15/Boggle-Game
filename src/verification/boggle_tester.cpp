...
void printVerificationResults(const VerificationResult& result) {
    ...
    if (result.allExpectedWordsFound && !result.extraWordsFound) {
        ...
        if (result.duplicatesFound) {
            std::cout << "Note: Some words were found multiple times through different paths." << std::endl;
        }
    } else {
        std::cout << "\nTest FAILED: There are discrepancies between expected and found words." << std::endl;
    }
}
