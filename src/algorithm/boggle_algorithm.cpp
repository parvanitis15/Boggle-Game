                                   ...
                                   std::string &currentWord, int r, int c, std::vector<std::string> &wordsFound)
{
    ...
    auto currentWordPlusNewLetter{currentWord + board.getLetter(static_cast<size_t>(r), static_cast<size_t>(c))};
    ...
    for (int r{rStart - 1}; r <= rStart + 1; r++)
    {
        for (int c{cStart - 1}; c <= cStart + 1; c++)
        {
            if (checkBoardIndex(wordsTrie, board, visitMap, currentWord, r, c, wordsFound) == ContinueTraversing::yes)
            {
                ...
                newVisitMap.markVisited(static_cast<size_t>(r), static_cast<size_t>(c)); // static_cast<size_t> is safe as long as the int is not negative and we've checked for that in bounds checking

                // Add the letter to the actual word for continuing traversal
                auto newCurrentWord{currentWord + board.getLetter(static_cast<size_t>(r), static_cast<size_t>(c))};

                traverseBoardRecursively(wordsTrie, board, newVisitMap, newCurrentWord, r, c, wordsFound);
            }
        }
    }
}
