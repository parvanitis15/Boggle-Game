...
// Function to check whether a board index letter, appended to current word, exists (and is a word) in the words Trie
// It also checks whether it has been visited before
...
// Important: currentWord is a new copy, because we take a path which we do not want to be alter the original word (which also takes other paths)
// Same with visitMap
